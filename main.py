...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str,default='paper',#'ACM',
                        help='Dataset')
    ...
    #     node_features = pickle.load(f)[:2219,:]
    node_features = np.load('./data/paper/emb_roberta_pca.npy')
    ...
    for i,edge in enumerate(edges):
        if i ==0:
            ...
            A_all = torch.from_numpy(edge.todense()).type(torch.FloatTensor)
        else:
            A = torch.cat([A,torch.from_numpy(edge.todense()).type(torch.FloatTensor).unsqueeze(-1)], dim=-1)
            A_all = A_all+torch.from_numpy(edge.todense()).type(torch.FloatTensor)
    ...
    for l in range(1):
        ...
        for i in range(epochs):
            ...
            model.zero_grad()
            model.train()
            loss,y_train,Ws,X_out = model(A, node_features, train_node, train_target,A_all)
            ...
            print(y_train)
        print(X_out)
        np.save('./data/embeddings.npy',X_out.detach().numpy())
        #     # Valid
        #     with torch.no_grad():
